...
public class Philosopher extends Thread {
	...
	public void  take_fork(int i) throws InterruptedException
	{
		mutex.acquire();
		state[i]=HUNGRY;
		table.Hungry_GUI(i);
	...
	public void run() {
		...
		int walk_time= rand.nextInt(10000)+1000; //sleep requires millisecond
		...
		//Barrier Implementation n times up/release  n-1 times down/acquire so that they wait for everyone to come table until start dining
		for (int i=0 ; i<N; i++)
		{
			if (id!=i)
			{
		...
		//Barrier is passed by all five philosophers now they can start dining
		table.StartDining_GUI(id);

		//generate random thinking time
		Random new_rand= new Random();
		int think_time= new_rand.nextInt(10000)+1000; //thinking time for philosophers 0-10 sec (10000 milliseconds)
		while (true)
		{
			try {
				sleep(think_time);
